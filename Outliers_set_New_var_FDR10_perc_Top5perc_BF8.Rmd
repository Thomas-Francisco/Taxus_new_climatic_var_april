...
```
7 Candidates were in LD with SNPs located on other contigs, we removed these LD association from the SNPs in LD (but still a SNP can be in LD with SNPs in the same contig so, it's possible that from these 7 SNPs, we still remove few of them)
...
```
Based on a LD of 0.7, we removed 38 SNPs. 
...
```
The final number of outliers retained for further analysis is 91.
