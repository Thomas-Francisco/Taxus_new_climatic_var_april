...
list_raster <- c("past_climatic_data_raster","present_climatic_raster","future_climatic_data_raster")
...
for(x in 1:length(list_raster)){
  ...
  load(paste0("C:/Users/tfrancisco/Documents/Thèse/Data/Espèces/Taxus_baccata/Genomic_offset/RDA/raster/",var,".Rdata")) #load data
...
```
    ...
    - one with only RDA outliers
```{r}
load("C:/Users/tfrancisco/Documents/Thèse/Results/species/taxus/GEA_new_var/outliers/outliers_RDA_FDR5perc_T_Adapcon_gentree.Rdata")

order_snp <- outliers_RDA_FDR5perc_T_Adapcon_gentree[order(as.numeric(as.character(outliers_RDA_FDR5perc_T_Adapcon_gentree$p.value))),]

set_rda_top <- order_snp[c(1:98),]
list_set_top_RDA <- set_rda_top$snp_names
```
...
```{r}
RDa_top_set <- rda(formula = genomic_matrix[list_set_top_RDA] ~  Annual_Tc+Diurnal_range_Tc+Tc_Seasonality+Tc_driest_quarter+Annual_P+P_Seasonality, data = Climatic_data_RDA_pRDA, scale=F)
```
...
```{r}
score_pop_real <- data.frame(scores(RDA_outliers_LC, choices=c(1,2), display="sites",scaling=0))
...
df <- merge(meta_data_pop,score_predict_pop,"Latitude")
...
test_prediction <- predict(RDA_outliers_LC, data, type = c("response", "wa", "sp", "lc", "working"),
        rank = 2, model = c("CCA"), scaling = "sites",
        hill = FALSE, ...)
```

  - Conservative outliers set
...
```
For further analysis, we will perform the analysis on the different outliers sets to see if we find the same patterns depending on the thresholds: 
```{r list set}
list_outliers_set <- c("list_random_SNPs","outliers_set_final_overlapping_no_LD_LC_new_var","outliers_set_final_overlapping_no_LD_new_var","list_set_top_RDA")
list_RDA <- c("RDA_random","RDA_outliers_LC","RDA_outliers_MC","RDA_top_set")
```

We can plot before the repartition of outliers along the RDA axis and the climatic variables: 

```{r screeplot outliers LC}
for(x in 1: length(list_RDA)){
  RDA <- get(list_RDA[x])
  name <- c("random_SNPs","outliers_Lc","Outliers_Mc","top_set")
  #screeplot
...
assign(paste0("explain_variance",name[x]),explain_variance)
  ...
  #save
#png(paste0("C:/Users/tfrancisco/Documents/Thèse/Results/species/taxus/Genomic_offset/RDA/figures/screeplot/Screeplot_RDA_",name[x],".png"));screeplot(RDA,main = paste0("Hist screeplot ",name[x]));dev.off()
}
```
We can see that the 2 first RDA axis seems to explain the major part of the genetic variation of the outliers for all the sets. We will use the 2 axis for further analysis for each sets. 


We can plot the loci and the climatic variables in the RDA space to visualize:
```{r plot biplot loci}
...
#pdf(paste0("C:/Users/tfrancisco/Documents/Thèse/Results/species/taxus/Genomic_offset/RDA/figures/biplot/biplot_outliers_RDA_",name[x],".pdf"));print(biplot_outliers_RDA);dev.off()
}
...
For the MC set, we can see that the pattern is the same but reverse for RDA first axis.
...
score_climatic_var <- as.data.frame(scores(RDA, choices=c(1:2), display="bp"))
Score_population <- data.frame(RDA$CCA$u[,c(1,2)])
...
#pdf(paste0("C:/Users/tfrancisco/Documents/Thèse/Results/species/taxus/Genomic_offset/RDA/figures/biplot/biplot_population_RDA_",name[x],".pdf"));print(biplot_populations);dev.off()
  
}

#score along the 2 first RDA axis
score_climatic_var <- as.data.frame(scores(RDA_outliers_LC, choices=c(1:2), display="bp"))


View(df)

#explained variance
explained_variance_round <- round(RDA_outliers_LC$CCA$eig*100/sum(RDA_outliers_LC$CCA$eig),digits=1)
...
biplot_populations <- ggplot() +
  geom_hline(yintercept = 0, linetype = "dashed", color = gray(0.80), size = 0.6) +
  geom_vline(xintercept = 0, linetype = "dashed", color = gray(0.80), size = 0.6) +
  geom_point(data = df, aes(x = RDA_pres_1 * 3, y = RDA_pres_2 * 3, colour = Country), size = 2, alpha = 0.8) +
  geom_segment(data = df, aes(xend = RDA_pres_1, yend = RDA_pres_2, x = 0, y = 0), colour = "black", size = 0.15, linetype = 1, arrow = arrow(length = unit(0.02, "npc"))) +
  geom_text(data = score_climatic_var, aes(x=1.1*RDA1, y=1.1*RDA2, label = row.names(score_climatic_var)), size = 2.5)+
  xlab(paste0("RDA 1 (",explained_variance_round[1],"%)")) + 
  ylab(paste0("RDA 2 (",explained_variance_round[2],"%)")) +
  ggtitle(paste0("Biplot RDA Populations ",name[x])) +
  scale_color_manual(name = "Countries", values = group_palette, labels = levels(df$Country)) +
  theme_bw(base_size = 11) +
  theme(legend.position="right", panel.grid = element_blank(), strip.text = element_text(size=11),plot.title = element_text(hjust = 0.5,color = "Black",face="italic"))+
  labs(color = "Country")
 ...
 observed_vs_predicted <- data.frame(
  observed_RDA1 = df_final_comp[,2],
  predicted_RDA1 = df_final_comp$RDA_pres_1,
  observed_RDA2 = df_final_comp[,3],
  predicted_RDA2 = df_final_comp$RDA_pres_2
)

 observed_vs_predicted_scaled <- data.frame(observed_vs_predicted %>% scale())
 
 plot(observed_vs_predicted_scaled$observed_RDA1, observed_vs_predicted_scaled$predicted_RDA1,
     xlab = "Observed RDA1", ylab = "Predicted RDA1", main = "Observed vs. Predicted RDA1")
abline(0, 1, col = "red")

 plot(observed_vs_predicted_scaled$observed_RDA2, observed_vs_predicted_scaled$predicted_RDA2,
     xlab = "Observed RDA1", ylab = "Predicted RDA1", main = "Observed vs. Predicted RDA1")
abline(0, 1, col = "red")
 
# Plot for visual comparison
plot(observed_vs_predicted$observed_RDA1, observed_vs_predicted$predicted_RDA1,
     xlab = "Observed RDA1", ylab = "Predicted RDA1", main = "Observed vs. Predicted RDA1")
abline(0, 1, col = "red")

plot(observed_vs_predicted$observed_RDA2, observed_vs_predicted$predicted_RDA2,
     xlab = "Observed RDA2", ylab = "Predicted RDA2", main = "Observed vs. Predicted RDA2")
abline(0, 1, col = "red")
 
```

We can see for the that the pattern we saw previously with the snps are in accordance here. Indeed, more continental or higher in altitude populations are on the left and the opposite pattern of population on the left. 
Same as before, for the MC outliers set, the pattern along RDA first axis is reverse but the information stayed the same.


        5. Interpolate/extrapolate the GEA relationship to space and time

The next step is to perform the interpolation/ extrapolation of the GEA relationship into time to estimate the past, present and future genomic composition for the outliers identified for the 29 populations.
We will calculate this into the past (time of establishment), present and future to visualize the evolution of treats
...
Adaptive index for past, present and future period:
...
```{r process and plot adaptive index}
#number of retained axis
K=2
  name_list <- c("RDA1","RDA2")
  list_past_index <- c("df_RDA_past_score_random","df_RDA_past_score_LC","df_RDA_past_score_MC")
  list_present_index <- c("df_RDA_present_score_random","df_RDA_present_score_LC","df_RDA_present_score_MC")
  list_future_index <- c("df_RDA_future_score_random","df_RDA_future_score_LC","df_RDA_future_score_MC")
  list_name <- c("random_SNPs","outliers_Lc","Outliers_Mc")
  
  for(i in 1:length(list_past_index)){
    
    past_score <- get(list_past_index[i])
    present_score <- get(list_present_index[i])
    future_score <- get(list_future_index[i])
  name <- list_name[i]
...
for(x in 1:K){
  ...
  var <- x+2
  axis_vector <- c(rep("Past", nrow(past_score)),rep("Present", nrow(present_score)), rep("Future", nrow(future_score)))
  RDA_df <- data.frame(axis=axis_vector,values=c(past_score[,var],present_score[,var],future_score[,var]),Longitude=future_score$Longitude,Latitude=future_score$Latitude)
...
RDA1_past_scale<- RDA1_list_scale$Past
RDA1_present_scale <- RDA1_list_scale$Present
RDA1_futur_scale <- RDA1_list_scale$Future

assign(paste0(names_df,"_Past_scale_",name),RDA1_past_scale)
assign(paste0(names_df,"_Present_scale_",name),RDA1_present_scale)
assign(paste0(names_df,"_Future_scale_",name),RDA1_futur_scale)
}
}
...
for(i in 1:K) {
  list_axis <- c("RDA1","RDA2","RDA3","RDA4","RDA5","RDA6")
    ...
    for(j in 1:length(list_period)){
      ...
      plot <- ggplot(data = data <- get(paste0(axis,"_",period,"_scale_",name))) + 
  ...
  ggtitle(paste0(period, " adaptive enriched ", axis, " space ",name)) +
  theme_bw(base_size = 11) +
  theme(legend.position="right", panel.grid = element_blank(), strip.text = element_text(size=11),plot.title = element_text(hjust = 0.5,color = "Black",face="italic"))
      
      print(plot)
      #save
      pdf(paste0("C:/Users/tfrancisco/Documents/Thèse/Results/species/taxus/Genomic_offset/RDA/figures/adaptive_index/",name,period,"_adaptive_enriched_",axis, "_space.pdf"));print(plot);dev.off()
    }
  }
}
...
We can run the GO function for both outliers set for present and future: 

```{r Run genomic_offset_pop random present}
#manual way
#run this function on our populations
#Run_genomic_offset_pop_random_present<- genomic_offset_pop(RDA_random,2,df_RDA_past_score_random,df_RDA_future_score_random,meta_data_pop)
#df
# genomic_offset_df_random_present<- data.frame(Population=unlist(Run_genomic_offset_pop_random_present$Population),Genomic_offset_random=unlist(Run_genomic_offset_pop_random_present$Proj_offset_global))
 #order the df per population
# Genomic_offset_Taxus_Adapcon_Gentree_RDA_random_present <- genomic_offset_df_random_present[order(genomic_offset_df_random_present$Population),]

list_RDA <- c("RDA_random","RDA_outliers_LC","RDA_outliers_MC")
list_past_score <- c("df_RDA_past_score_random","df_RDA_past_score_LC","df_RDA_past_score_MC","df_RDA_past_score_random","df_RDA_past_score_LC","df_RDA_past_score_MC")
list_present_future_score <- c("df_RDA_present_score_random","df_RDA_present_score_LC","df_RDA_present_score_MC","df_RDA_future_score_random","df_RDA_future_score_LC","df_RDA_future_score_MC")
list_name <- c("random_present","LC_present","MC_present","random_future","LC_future","MC_future")  
  
for(i in 1:length(list_RDA)){
RDA <- get(list_RDA[i])

  for(x in 1:length(list_past_score)){
  adaptive_past_score <- get(list_past_score[x])
  present_future_score <- get(list_present_future_score[x])
  name <- list_name[x]
  
  #run this function on our populations
Run_genomic_offset_pop<- genomic_offset_pop(RDA,2,adaptive_past_score,present_future_score,meta_data_pop)
#df
 ...
 assign(paste0("Genomic_offset_Taxus_Adapcon_Gentree_RDA_",name),Genomic_offset_Taxus_Adapcon_Gentree_RDA)
  }
}
...
list_GO <- c("Genomic_offset_Taxus_Adapcon_Gentree_RDA_random_present","Genomic_offset_Taxus_Adapcon_Gentree_RDA_LC_present","Genomic_offset_Taxus_Adapcon_Gentree_RDA_MC_present","Genomic_offset_Taxus_Adapcon_Gentree_RDA_random_future","Genomic_offset_Taxus_Adapcon_Gentree_RDA_LC_future","Genomic_offset_Taxus_Adapcon_Gentree_RDA_MC_future")
for(x in 1:length(list_GO)){
   file_save<- get(list_GO[x])
  name <- list_GO[x]
  assign(name,file_save)
    save(list = name,file=paste0("C:/Users/tfrancisco/Documents/Thèse/Results/species/taxus/Genomic_offset/RDA/data/",name,".Rdata"))
}

```

We can plot the genomic offset values for each populations: 
Such as previously, we can apply the cut_number function that will rescale the values in the same color scale as other GO like GF. 
```{r plot genomic offset}

list_GO <- c("Genomic_offset_Taxus_Adapcon_Gentree_RDA_random","Genomic_offset_Taxus_Adapcon_Gentree_RDA_LC","Genomic_offset_Taxus_Adapcon_Gentree_RDA_MC")
...
for(i in 1:length(list_period)){
  ...
  for(x in 1: length(list_set)){
    ...
    set <- list_set[x]
    GO_df <- get(paste0("Genomic_offset_Taxus_Adapcon_Gentree_RDA_",set,"_",period))
    
     #first, we need to add the coordinates
Genomic_offset_coord <- merge(GO_df,meta_data_pop[,c(2,4,5)],"Population")
...
#save
     pdf(paste0("C:/Users/tfrancisco/Documents/Thèse/Results/species/taxus/Genomic_offset/RDA/figures/GO/Genomic offset across populations ",set," ",period,".pdf"));print(plot);dev.off()
...
for(x in 1:length(list_GO)){
  GO_df <- get(list_GO[x])
  name <- c("Random","Outliers_Lc","Outliers_Mc")
...
plot <- ggplot(data = Genomic_offset_coord) + 
  ...
  xlab("Longitude") + ylab("Latitude") +
  guides(fill = guide_legend(title = "Genomic offset")) +
  ggtitle(paste0("Genomic offset across populations ",name[x])) +
  theme_bw(base_size = 11) +
  theme(legend.position="right", panel.grid = element_blank(), strip.text = element_text(size=11),plot.title = element_text(hjust = 0.5,color = "Black",face="italic"))
...
#save
     pdf(paste0("C:/Users/tfrancisco/Documents/Thèse/Results/species/taxus/Genomic_offset/RDA/figures/GO/Genomic_offset_across_populations_",name[x],".pdf"));print(plot);dev.off()
}
```


Now, we can investigate the correlation between the differente GO predictions and also We can see as for GF that there is not a clear pattern of maladaptation. Some populations closed to the sea exhibit high vlaues of genomic offset, others not and same things for continentality.
We wanted to see if the elevation could have an impact on the maladaptation. 
...
data_GO_altitude <- data.frame(merge(Genomic_offset_Taxus_Adapcon_Gentree_RDA_random_present,elevation_data,"Population"))
...
#df
df_tot_GO_predictions <- data.frame(Population=Genomic_offset_Taxus_Adapcon_Gentree_RDA_random_present[,1],GO_rand_pres=Genomic_offset_Taxus_Adapcon_Gentree_RDA_random_present[,2],GO_rand_Fut=Genomic_offset_Taxus_Adapcon_Gentree_RDA_random_future[,2],GO_LC_pres=Genomic_offset_Taxus_Adapcon_Gentree_RDA_LC_present[,2],GO_LC_Fut=Genomic_offset_Taxus_Adapcon_Gentree_RDA_LC_future[,2],GO_MC_pres=Genomic_offset_Taxus_Adapcon_Gentree_RDA_MC_present[,2],GO_MC_Fut=Genomic_offset_Taxus_Adapcon_Gentree_RDA_MC_future[,2],elevation=data_GO_altitude_df$elevation)
...
```
```{r save corrplot}
pdf("C:/Users/tfrancisco/Documents/Thèse/Results/species/taxus/Genomic_offset/RDA/figures/comparison/correlation_GO_values_GF_T_set_period.pdf");corrplot(correlation, method = "number", addrect = 2, col = c("darkorange","darkred"), type = "lower", tl.col = "black", tl.cex = 0.6, number.cex = 0.6);dev.off()
```


We can see that there is almost no correlation between GO values and elevation. To see that, we will plot the elevation to the points in the genomic offset map: 
...
plot <- ggplot(data = data_GO_altitude_df) + 
  geom_sf(data = admin, fill = gray(0.92), size = 0) +
  geom_point(aes(x = Longitude, y = Latitude, fill = cut_number(Genomic_offset_LC, n = 5), size = elevation), shape = 21, color = "black") +
  scale_fill_manual(
    values = colors,
    labels = c("low values", "", "", "", "high values"),
  ...
  ggtitle("RDA Genomic offset and Elevation across populations") +
  theme_bw(base_size = 11) +
  theme(
...
```{r include=FALSE}
#save
pdf("C:/Users/tfrancisco/Documents/Thèse/Results/species/taxus/Genomic_offset/RDA/figures/Genomic_offset_across_population_Taxus_elevation.pdf");print(plot);dev.off()
```
...
for(i in 1:length(list_set)){
  ...
  df_pres <- get(paste0("Genomic_offset_Taxus_Adapcon_Gentree_RDA_",set,"_",period_1))
  df_fut <- get(paste0("Genomic_offset_Taxus_Adapcon_Gentree_RDA_",set,"_",period_2))
...
We can ask ourself that maybe the values are similar but the rank of population is not and vice versa, we will investigate that: 
...
for(i in 1:length(list_period)){
  ...
  for(x in 1:(length(list_set)-1)){
    ...
    #merge two dataset
GO_RDA_merge <- merge(get(paste0("Genomic_offset_Taxus_Adapcon_Gentree_RDA_",set,"_",period)),get(paste0("Genomic_offset_Taxus_Adapcon_Gentree_RDA_",set2,"_",period)),"Population")
...
#plot
GO_RDA_set_df_meta <- merge(GO_RDA_set_df,meta_data_pop[,c(1,2)],"Population")
...
pdf(paste0("C:/Users/tfrancisco/Documents/Thèse/Results/species/taxus/Genomic_offset/RDA/figures/comparison/Scatterplot_comparison_rank_pop_",set,"_",set2,"_",period,"_Taxus.pdf"));print(Scatterplot);dev.off()
  }
}
```
